...
import { ANSWER_WORDS, MIN_WORD_LENGTH, MAX_WORD_LENGTH } from "./word500-answers.js";
import { dictionaryState, loadDictionary, isValidGuessWord } from "./word500-dictionary.js";
import { buildDifficultyIndex, getWordsForDifficulty } from "./word500-difficulty.js";
...
//
//   import { mountWord500 } from "./games/word500/word500-server.js";
//   mountWord500(app, httpServer);
//
...
// http.createServer(app) returns. WORD500 is entirely namespaced
// under `mountPath` (default "/games/word500") for its HTTP routes
// and static assets, and under `wsPath` (default "/word500-ws") for
...
// wsPath (this file's default of "/word500-ws" is already
...
export function mountWord500(app, httpServer, options = {}) {
  const mountPath = options.mountPath || "/games/word500";
  const wsPath = options.wsPath || "/word500-ws";

  app.use(mountPath, express.static(path.join(__dirname, "public")));
  app.get(mountPath, (req, res) => {
    res.sendFile(path.join(__dirname, "public", "word500-index.html"));
  });
  ...
  loadDictionary();
  console.log(
    `[WORD500] Mounted at ${mountPath} (WebSocket at ${wsPath}). ` +
...
// Standalone mode
// ============================================================
// Lets you test this game entirely on its own (`node word500-server.js`)
...
// mountWord500 above - importing it (the normal merge path) never
...
if (isRunDirectly) {
  ...
  mountWord500(standaloneApp, standaloneServer, { mountPath: "/", wsPath: "/word500-ws" });
  const PORT = process.env.PORT || 3000;
  standaloneServer.listen(PORT, () => {
    console.log(`[WORD500] Standalone test server listening on port ${PORT}`);
  });
}
